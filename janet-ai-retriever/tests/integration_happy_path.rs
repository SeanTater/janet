...
async fn test_file_indexing_without_embeddings() -> Result<()> {
    ...
    loop {
        ...
        if attempts >= max_attempts {
            panic!(
                "Timeout waiting for indexing to complete. Queue size: {}, Files processed: {}",
    ...
    println!(
        "Final stats: Files processed: {}, Chunks created: {}",
        final_stats.files_processed, final_stats.chunks_created
    );
    println!(
        "Index stats: Files: {}, Chunks: {}",
    ...
    assert!(
        final_stats.chunks_created > 0,
        "Expected chunks to be created, got {}",
        final_stats.chunks_created
    );

    // Index should contain the files and chunks
    assert!(
        index_stats.files_count >= 4,
        "Expected at least 4 files in index, got {}",
        index_stats.files_count
    );
    assert!(
        index_stats.chunks_count > 0,
        "Expected chunks in index, got {}",
        index_stats.chunks_count
    );
...
async fn test_read_only_mode() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Expected file indexing to fail in read-only mode"
    );
    ...
    assert!(
        result.is_err(),
        "Expected full reindex to fail in read-only mode"
    );
    ...
    loop {
        ...
        if stats.files_processed > 0 {
            ...
            assert!(
                ...
                "Expected multiple chunks for large file, got {}",
