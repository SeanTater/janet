...
impl FileIndex {
    ...
    fn file_refs(&self) -> Result<fjall::PartitionHandle> {
        Ok(self
            .keyspace
            .open_partition("FileRef", fjall::PartitionCreateOptions::default())?)
    }

    fn write_file_ref(&self, file_ref: &FileRef) -> Result<()> {
        let refs = self.file_refs()?;
        refs.insert(&file_ref.hash, &bitcode::encode(file_ref))?;
        Ok(())
    }
}
...
impl DirectoryTracker {
    pub async fn open(path: Option<&Path>, analyzer: Analyzer) -> Result<Self> {
